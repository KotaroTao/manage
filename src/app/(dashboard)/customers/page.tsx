...
import { formatDate, getApiError, exportToCsv } from '@/lib/utils';
    ...
    <div className="p-4 md:p-6 max-w-7xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          <button
            type="button"
            onClick={() => exportToCsv('customers', ['顧客名', '会社名', 'メール', '電話番号', 'ステータス', '事業数', '登録日'], customers.map((c) => [c.name, c.company, c.email, c.phone, c.status === 'ACTIVE' ? '有効' : '無効', c._count?.customerBusinesses ?? 0, formatDate(c.createdAt)]))}
